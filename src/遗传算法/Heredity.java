...
public class Heredity {
    ...
    public void crossover(int x, int y)//交叉
    {
        ...
        if (Math.random() <= pxover) {
            ...
            for (int i = 1; i <= genelenth; i++) {
                ...
                } else if ((jcross > 1) && (jcross <= genelenth)) {
    ...
    public static void main(String[] args)//主函数
    {
        ...
        while (test.maxgens > 0) {
            ...
            double number = get10(test.genelenth, test.chom,0);

            for (int i = 0; i < test.popsize; i++){
                if (test.chom[i].fitness < temp){
                    temp = test.chom[i].fitness;
                    number = get10(test.genelenth, test.chom,i);
                }

            }

            System.out.println("----------第" + dai + "代-----------   " + temp + "  " + number);
    ...
    public static double get10(int genelenth, chromosome chom1[], int index){
        ...
        for (int j = 0; j < genelenth; j++) {
            k += chom1[index].gene[j] * Math.pow(2.0, (double) j);
        }
